// 1 TAB = 8 SPACES //
	...
	*	|										|
	*	|										|
	*	|	STOP():		Must send a stop sequence after the current operation	|
	*	|			has finished (be it a START+WR, a TX or a RX), unless	|
	*	|			it is a START+RD, in that case the STOP will not be	|
	*	|			inmediately sent, instead it will wait for the START+RD	|
	*	|			to end and be sent after the reception of the first	|
	*	|			byte (RX).							|
	*	|										|
	*	+-------------------------------------------------------------------------------+
	*	
